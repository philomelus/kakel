#pragma warning disable RCS1213, IDE0051, RCS1110, RCS1146, IDE0044, RCS1169, IDE0052, IDE0059
...
public partial class Preferences : Node
{
    ...
    public void Load(string path)
    {
        ...
        if (err != Error.Ok && err != Error.FileNotFound)
            throw new ArgumentException($"Unable to read configuration: {err}");
        if ((long) cf.GetValue(S_GLOBALS, V_VERSION, PREFS_VERSION) != PREFS_VERSION)
             throw new ArgumentOutOfRangeException("Unknown preferences version");
        AutoLoad = (long) cf.GetValue(S_GLOBALS, V_AUTOLOAD, default_AutoLoad ? 1 : 0) == 1;
        AutoPath = (string) cf.GetValue(S_GLOBALS, V_AUTOPATH, default_AutoPath);
        AutoRemoveOnWin = (long) cf.GetValue(S_GLOBALS, V_AUTOREMOVEONWIN, default_AutoRemoveOnWin ? 1 : 0) == 1;
        AutoSave = (long) cf.GetValue(S_GLOBALS, V_AUTOSAVE, default_AutoSave ? 1 : 0) == 1;
        Columns = (int) (long) cf.GetValue(S_GLOBALS, V_COLUMNS, default_Columns);
        ...
        Rows = (int) (long) cf.GetValue(S_GLOBALS, V_ROWS, default_Rows);
        ShowOutlines = (long) cf.GetValue(S_GLOBALS, V_SHOWOUTLINES, default_ShowOutlines ? 1 : 0) == 1;
        ShowNumbers = (long) cf.GetValue(S_GLOBALS, V_SHOWNUMBERS, default_ShowNumbers ? 1 : 0) == 1;
    }
