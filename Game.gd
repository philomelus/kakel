...
func _on_hint_pressed() -> void:
	...
	if _hint_image.texture == null:
		...
		_hint_image.texture = ImageTexture.create_from_image(image)
...
func _on_tiles_won() -> void:
	...
	if Globals.auto_started and Globals.preferences.auto_remove_on_win:
		if FileAccess.file_exists(Globals.preferences.auto_path):
			var _x = DirAccess.remove_absolute(Globals.preferences.auto_path)
...
func _ready() -> void:
	...
	var pm: PopupMenu = _prefs.get_popup()
	var _x = pm.connect("index_pressed", _on_prefs_item_selected.bind())
