...
public class TilesControl : Control
{
    ...
    public void RecalcTiles()
    {
		...
		if (UseImage)
		{
			if (_image == null || RectSize.x != _image.GetWidth() || RectSize.y != _image.GetHeight())
			{
				_image = LoadImage(_imagePath);
				_image.Resize((int) RectSize.x, (int) RectSize.y);
			}
            ImageTexture it = new ImageTexture();
            it.CreateFromImage(_image);
			_tilesTexture = it;
		}

		// Determine width and height of tiles from our size.
		_tileSize = new Vector2((RectSize.x - (_columns0 * _spacing.x)) / _columns,
